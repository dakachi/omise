...
class MS_Gateway_Omise_Api extends MS_Model_Option {
	...
	public function get_omise_customer( $member, $token ) {
		...
		if ( empty( $customer ) ) {
			$customer = M2_omise_Customer::create(
				array(
					'card' => $token,
					'email' => $member->email,
				)
	...
	public function find_customer( $member ) {
		...
		if ( ! empty( $customer_id ) ) {
			$customer = M2_omise_Customer::retrieve( $customer_id );
	 ...
	 * @param M2_omise_Customer $customer The omise customer object.
	 ...
	 * @internal
	 *
	 * @param  M2_omise_Customer $customer omise customer to charge.
	...
	public function get_subscription( $customer, $membership ) {
		$plan_id = MS_Gateway_omiseplan::get_the_id(
			$membership->id,
			'plan'
		);
	...
	public function subscribe( $customer, $invoice ) {
		...
		if ( ! $subscription ) {
			...
			if ( $invoice->coupon_id ) {
				$coupon_id = MS_Gateway_omiseplan::get_the_id(
					$invoice->coupon_id,
					'coupon'
				);
			}
	...
	public function create_or_update_plan( $plan_data ) {
		...
		if ( ! isset( $all_items[$item_id] )
			|| ! is_a( $all_items[$item_id], 'M2_omise_Plan' )
		) {
			try {
				$item = M2_omise_Plan::retrieve( $item_id );
			} catch( Exception $e ) {
		 ...
		 */
		if ( $item && is_a( $item, 'M2_omise_Plan' ) ) {
			$item->delete();
			$all_items[$item_id] = false;
		}

		if ( $plan_data['amount'] > 0 ) {
			$item = M2_omise_Plan::create( $plan_data );
			$all_items[$item_id] = $item;
		}
	...
	public function create_or_update_coupon( $coupon_data ) {
		...
		if ( ! isset( $all_items[$item_id] )
			|| ! is_a( $all_items[$item_id], 'M2_omise_Coupon' )
		) {
			try {
				$item = M2_omise_Coupon::retrieve( $item_id );
			} catch( Exception $e ) {
				// If the coupon does not exist then omise will throw an Exception.
				$item = false;
			}
			$all_items[$item_id] = $item;
		} else {
		...
		/*
		 * omise can only update the coupon-name, so we have to delete and
		 * recreate the coupon manually.
		 */
		if ( $item && is_a( $item, 'M2_omise_Coupon' ) ) {
			$item->delete();
			$all_items[$item_id] = false;
		}

		$item = M2_omise_Coupon::create( $coupon_data );
		$all_items[$item_id] = $item;

		MS_Factory::set_transient(
			'ms_omiseplan_coupons',
			$all_items,
			HOUR_IN_SECONDS
		);
	}
